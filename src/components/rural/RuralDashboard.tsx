...
import { 
  Wheat, Tractor, MapPin, Sprout,
  Users, Activity, Award, Flame,
  Wind, Plus, ArrowUpRight
} from "lucide-react"
...
export function RuralDashboard() {
  ...
  const [activeTab, setActiveTab] = useState<'overview' | 'farmer_hub'>('overview')
    ...
    <div className="p-4 pb-36 md:p-8 space-y-10 animate-in fade-in duration-700 min-h-screen bg-[#f8fafc] dark:bg-[#020617] text-foreground">
      ...
      <div className="flex border-b border-white/10 pb-px gap-2">
        <button
          ...
          className={cn(
            "pb-4 px-6 text-sm font-bold uppercase tracking-wider relative transition-all duration-300 flex items-center gap-2",
            activeTab === 'overview' 
              ? "text-emerald-500 font-black border-b-2 border-emerald-500 dark:text-emerald-400" 
              : "text-white/40 hover:text-white/80"
          )}
        >
          <Activity size={16} /> {t("rural_overview_tab")}
        </button>
        <button
          onClick={() => setActiveTab('farmer_hub')}
          className={cn(
            "pb-4 px-6 text-sm font-bold uppercase tracking-wider relative transition-all duration-300 flex items-center gap-2",
        ...
        >
          <Sprout size={16} /> {t("rural_hub_tab")}
        </button>
      </div>
      ...
      {activeTab === 'overview' ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
             <div className="p-8 bg-card border border-border rounded-[2.5rem] flex flex-col justify-between group hover:border-emerald-500/30 transition-all">
                ...
                <div>
                   ...
                   <button 
                     onClick={() => setActiveTab('farmer_hub')}
                     className="w-full py-3 bg-emerald-500 text-white rounded-2xl font-bold text-xs hover:opacity-90 transition-all"
                   >
                     Sell Waste →
                   </button>
                </div>
             ...
             <div className="p-8 bg-card border border-border rounded-[2.5rem] flex flex-col justify-between group hover:border-blue-500/30 transition-all">
                ...
                <div>
                   ...
                   <button 
                     onClick={() => setActiveTab('farmer_hub')}
                     className="w-full py-3 bg-blue-500 text-white rounded-2xl font-bold text-xs hover:opacity-90 transition-all"
                   >
                     Book Pickup →
                   </button>
                </div>
        ...
        <div className="animate-in fade-in slide-in-from-bottom-6 duration-700 bg-black/30 rounded-[2.5rem] border border-white/5 p-6 md:p-10 shadow-2xl">
          <FarmerDashboard />
        </div>
      )}
    </div>
