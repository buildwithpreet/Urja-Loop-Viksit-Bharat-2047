-- URJALOOP SUPABASE SCHEMA
-- For Viksit Bharat Hackathon 2047

-- 1. PROFILES (Extends Auth.Users)
CREATE TABLE IF NOT EXISTS public.profiles (
  id UUID REFERENCES auth.users NOT NULL PRIMARY KEY,
  full_name TEXT,
  phone TEXT UNIQUE,
  location TEXT,
  eco_credits BIGINT DEFAULT 0,
  co2_saved DECIMAL DEFAULT 0,
  waste_processed DECIMAL DEFAULT 0, -- in kg
  role TEXT DEFAULT 'citizen', -- citizen, admin, collector, farmer
  created_at TIMESTAMP WITH TIME ZONE DEFAULT timezone('utc'::text, now()) NOT NULL
);

-- 2. SMART BINS
CREATE TABLE IF NOT EXISTS public.smart_bins (
  id BIGINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
  name TEXT NOT NULL,
  location_name TEXT,
  lat DOUBLE PRECISION,
  lng DOUBLE PRECISION,
  fill_level INT DEFAULT 0, -- 0 to 100
  status TEXT DEFAULT 'active', -- active, full, maintenance
  last_cleaned TIMESTAMP WITH TIME ZONE,
  type TEXT DEFAULT 'general', -- general, plastic, organic, hazard
  created_at TIMESTAMP WITH TIME ZONE DEFAULT timezone('utc'::text, now()) NOT NULL
);

-- 3. COMPLAINTS
CREATE TABLE IF NOT EXISTS public.complaints (
  id TEXT PRIMARY KEY, -- CMP-XXX
  user_id UUID REFERENCES auth.users,
  type TEXT NOT NULL,
  location_name TEXT,
  lat DOUBLE PRECISION,
  lng DOUBLE PRECISION,
  severity TEXT DEFAULT 'Medium',
  description TEXT,
  status TEXT DEFAULT 'Submitted', -- Submitted, Under Review, Assigned, In Progress, Resolved
  status_idx INT DEFAULT 0,
  assigned_to TEXT,
  expected_resolution TEXT,
  image_url TEXT,
  ai_validated BOOLEAN DEFAULT false,
  created_at TIMESTAMP WITH TIME ZONE DEFAULT timezone('utc'::text, now()) NOT NULL
);

-- 4. MARKETPLACE ITEMS
CREATE TABLE IF NOT EXISTS public.marketplace_items (
  id BIGINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
  name TEXT NOT NULL,
  description TEXT,
  price INT NOT NULL, -- in credits
  image TEXT,
  category TEXT, -- Energy, Fertilizer, Raw Material, Biomass
  type TEXT, -- urban, rural
  stock INT DEFAULT 10,
  created_at TIMESTAMP WITH TIME ZONE DEFAULT timezone('utc'::text, now()) NOT NULL
);

-- 5. ACTIVITY LOG (For Dashboard Feed)
CREATE TABLE IF NOT EXISTS public.activity_log (
  id BIGINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
  user_id UUID REFERENCES auth.users,
  action TEXT NOT NULL, -- "Scanned Waste", "Earned Credits", "Filed Complaint"
  description TEXT,
  points_earned INT DEFAULT 0,
  created_at TIMESTAMP WITH TIME ZONE DEFAULT timezone('utc'::text, now()) NOT NULL
);

-- RLS POLICIES (Row Level Security)
ALTER TABLE public.profiles ENABLE ROW LEVEL SECURITY;
ALTER TABLE public.smart_bins ENABLE ROW LEVEL SECURITY;
ALTER TABLE public.complaints ENABLE ROW LEVEL SECURITY;
ALTER TABLE public.marketplace_items ENABLE ROW LEVEL SECURITY;
ALTER TABLE public.activity_log ENABLE ROW LEVEL SECURITY;

-- Public read access for Bins and Marketplace
CREATE POLICY "Public Bins Read" ON public.smart_bins FOR SELECT USING (true);
CREATE POLICY "Public Marketplace Read" ON public.marketplace_items FOR SELECT USING (true);

-- 6. FUNCTIONS & TRIGGERS

-- A. AUTO-CREATE PROFILE ON SIGNUP
CREATE OR REPLACE FUNCTION public.handle_new_user()
RETURNS TRIGGER AS $$
BEGIN
  INSERT INTO public.profiles (id, full_name, role)
  VALUES (new.id, new.raw_user_meta_data->>'full_name', 'citizen');
  RETURN new;
END;
$$ LANGUAGE plpgsql SECURITY DEFINER;

CREATE TRIGGER on_auth_user_created
  AFTER INSERT ON auth.users
  FOR EACH ROW EXECUTE FUNCTION public.handle_new_user();

-- B. AUTO-LOG COMPLAINTS
CREATE OR REPLACE FUNCTION public.log_new_complaint()
RETURNS TRIGGER AS $$
BEGIN
  INSERT INTO public.activity_log (user_id, action, description, points_earned)
  VALUES (new.user_id, 'Filed Complaint', 'Reported ' || new.type || ' at ' || new.location_name, 0);
  RETURN new;
END;
$$ LANGUAGE plpgsql SECURITY DEFINER;

CREATE TRIGGER on_complaint_filed
  AFTER INSERT ON public.complaints
  FOR EACH ROW EXECUTE FUNCTION public.log_new_complaint();

-- 7. COMMUNITY METRICS & VIEWS

-- A. GLOBAL IMPACT VIEW
CREATE OR REPLACE VIEW public.community_stats AS
SELECT 
  COALESCE(SUM(waste_processed), 0) as total_waste_kg,
  COALESCE(SUM(co2_saved), 0) as total_co2_kg,
  COALESCE(SUM(eco_credits), 0) as total_community_credits,
  COUNT(id) as total_users
FROM public.profiles;

-- B. INFRASTRUCTURE HEALTH VIEW
CREATE OR REPLACE VIEW public.infrastructure_stats AS
SELECT 
  COUNT(id) as total_bins,
  COALESCE(AVG(fill_level), 0) as avg_fill_level,
  COUNT(id) FILTER (WHERE status = 'active') as active_bins,
  COUNT(id) FILTER (WHERE fill_level > 80) as critical_bins
FROM public.smart_bins;

-- 8. RURAL ECOSYSTEM
CREATE TABLE IF NOT EXISTS public.collection_centers (
  id BIGINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
  name TEXT NOT NULL,
  location_name TEXT,
  lat DOUBLE PRECISION,
  lng DOUBLE PRECISION,
  capacity DECIMAL, -- in tons
  current_stock DECIMAL DEFAULT 0,
  type TEXT DEFAULT 'biomass', -- biomass, plastic, electronic
  created_at TIMESTAMP WITH TIME ZONE DEFAULT timezone('utc'::text, now()) NOT NULL
);

CREATE TABLE IF NOT EXISTS public.pickup_requests (
  id TEXT PRIMARY KEY, -- REQ-XXX
  user_id UUID REFERENCES auth.users,
  waste_type TEXT NOT NULL,
  estimated_weight DECIMAL,
  location_name TEXT,
  lat DOUBLE PRECISION,
  lng DOUBLE PRECISION,
  status TEXT DEFAULT 'Pending', -- Pending, Assigned, In Transit, Completed
  assigned_vehicle_id BIGINT,
  collection_center_id BIGINT REFERENCES public.collection_centers,
  image_url TEXT,
  created_at TIMESTAMP WITH TIME ZONE DEFAULT timezone('utc'::text, now()) NOT NULL
);

-- 9. FLEET MANAGEMENT
CREATE TABLE IF NOT EXISTS public.vehicles (
  id BIGINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
  plate_number TEXT UNIQUE NOT NULL,
  type TEXT NOT NULL, -- urban_truck, rural_tractor, electric_van
  current_lat DOUBLE PRECISION,
  current_lng DOUBLE PRECISION,
  status TEXT DEFAULT 'idle', -- idle, on_route, maintenance
  driver_name TEXT,
  last_ping TIMESTAMP WITH TIME ZONE DEFAULT timezone('utc'::text, now()) NOT NULL
);

-- 10. NOTIFICATIONS
CREATE TABLE IF NOT EXISTS public.notifications (
  id BIGINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
  user_id UUID REFERENCES auth.users,
  title TEXT NOT NULL,
  message TEXT NOT NULL,
  type TEXT DEFAULT 'info', -- info, success, warning, alert
  is_read BOOLEAN DEFAULT false,
  created_at TIMESTAMP WITH TIME ZONE DEFAULT timezone('utc'::text, now()) NOT NULL
);

-- 11. AI & CHATBOT LOGS
CREATE TABLE IF NOT EXISTS public.chatbot_logs (
  id BIGINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
  user_id UUID REFERENCES auth.users,
  query TEXT NOT NULL,
  response TEXT NOT NULL,
  metadata JSONB,
  created_at TIMESTAMP WITH TIME ZONE DEFAULT timezone('utc'::text, now()) NOT NULL
);

CREATE TABLE IF NOT EXISTS public.ai_analysis (
  id BIGINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
  user_id UUID REFERENCES auth.users,
  image_url TEXT,
  result JSONB, -- { type, category, confidence, credits }
  created_at TIMESTAMP WITH TIME ZONE DEFAULT timezone('utc'::text, now()) NOT NULL
);

-- 12. PROCESSED OUTPUTS (For Marketplace)
CREATE TABLE IF NOT EXISTS public.processed_outputs (
  id BIGINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
  name TEXT NOT NULL,
  description TEXT,
  category TEXT, -- Biofuel, Fertilizer, Recycled Plastic
  price_per_unit INT, -- in credits
  unit TEXT, -- kg, ton, liter
  stock DECIMAL DEFAULT 0,
  image_url TEXT,
  created_at TIMESTAMP WITH TIME ZONE DEFAULT timezone('utc'::text, now()) NOT NULL
);

-- RLS for new tables
ALTER TABLE public.collection_centers ENABLE ROW LEVEL SECURITY;
ALTER TABLE public.pickup_requests ENABLE ROW LEVEL SECURITY;
ALTER TABLE public.vehicles ENABLE ROW LEVEL SECURITY;
ALTER TABLE public.notifications ENABLE ROW LEVEL SECURITY;
ALTER TABLE public.chatbot_logs ENABLE ROW LEVEL SECURITY;
ALTER TABLE public.ai_analysis ENABLE ROW LEVEL SECURITY;
ALTER TABLE public.processed_outputs ENABLE ROW LEVEL SECURITY;

CREATE POLICY "Public Centers Read" ON public.collection_centers FOR SELECT USING (true);
CREATE POLICY "Public Vehicles Read" ON public.vehicles FOR SELECT USING (true);
CREATE POLICY "Public Processed Outputs Read" ON public.processed_outputs FOR SELECT USING (true);
CREATE POLICY "User Requests Access" ON public.pickup_requests FOR ALL USING (auth.uid() = user_id);
CREATE POLICY "User Notifications Access" ON public.notifications FOR ALL USING (auth.uid() = user_id);
CREATE POLICY "User AI Logs Access" ON public.ai_analysis FOR ALL USING (auth.uid() = user_id);

-- GRANTs
GRANT SELECT ON public.collection_centers TO anon, authenticated;
GRANT SELECT ON public.vehicles TO anon, authenticated;
GRANT SELECT ON public.processed_outputs TO anon, authenticated;
